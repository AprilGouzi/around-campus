...
public class FollowServiceImpl extends ServiceImpl<FollowMapper, Follow> implements IFollowService {
    ...
    public Result follow(Long userFollowId, boolean isFollow) {
        ...
        if (isFollow) {
            ...
            if (isSuccess) {
                //把关注用户的id ，放入redis 的 set 集合 sadd userId followerUserId
                stringRedisTemplate.opsForSet().add(key, userFollowId.toString());
            }
            save(follow);
        } else {
            boolean isSuccess = remove(new QueryWrapper<Follow>().eq("user_id", userId).eq("follow_user_id", userFollowId));
            if (isSuccess) {
                //把关注用户的id 从Redis 集合中移除
